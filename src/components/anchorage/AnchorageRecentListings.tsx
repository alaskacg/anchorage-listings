...
import { MapPin, Clock, ChevronDown, Loader2 } from "lucide-react";
...
// Anchorage metro regions for filtering
...
const AnchorageRecentListings = () => {
  ...
  const { data: listings, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Filter for Anchorage metro regions
      return data?.filter(listing => 
        anchorageRegions.some(region => 
          listing.region?.toLowerCase().includes(region)
        )
      ) || [];
    },
  });
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="text-center mb-10">
          ...
          <h2 className="font-display text-2xl md:text-3xl lg:text-4xl font-bold text-foreground mb-3">
            Latest From Greater Anchorage
          </h2>
          <p className="text-muted-foreground max-w-2xl mx-auto">
            {totalListings > 0 
              ? `Showing ${displayedListings.length} of ${totalListings} active Anchorage listings`
              : "Fresh listings from the Anchorage metro area"
            }
          </p>
        </div>
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
              {displayedListings.map((listing, index) => (
                <Link
                  key={listing.id}
                  to={`/anchorage/listing/${listing.id}`}
          ...
          <div className="text-center py-16 bg-card rounded-xl border border-border/30">
            ...
            <h3 className="font-display text-xl font-semibold text-foreground mb-2">No Anchorage Listings Yet</h3>
            <p className="text-muted-foreground mb-6 max-w-md mx-auto">
              Be the first to post a listing on Anchorage's premier private marketplace
            </p>
...
export default AnchorageRecentListings;
